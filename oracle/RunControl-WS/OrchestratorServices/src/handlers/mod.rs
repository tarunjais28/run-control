...
use db::{get_last_assigned_run_id, get_lock_status};
...
pub fn home(
    ...
    let trigger = TriggerInfo {
        run_id: get_last_assigned_run_id(path.0, db.clone()).unwrap_or(0) + 1,
        trigger: RunInfo {
    ...
    let is_locked: String =
        get_lock_status(&trigger.trigger.batch_id, &as_on_date.and_hms(0, 0, 0), &db);
    if is_locked == "N" {
        thread::spawn(move || {
