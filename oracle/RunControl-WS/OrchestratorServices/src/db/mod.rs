...
pub fn get_last_assigned_run_id(
    ...
    match conn {
        Some(db) => {
            let sql = "select max(RunID) as RunID from RunControl where PoolID = :1 ";
            let rows = db
                .conn
                .query(sql, &[&pool_id])
...
pub fn get_triggerable_stream_ids(
    ...
    for id in trigger_info.trigger.stream_ids.clone() {
        ...
        match conn {
            Some(db) => {
                let sql = "select StreamDep from BATCHSTREAM where BatchID = :1 AND StreamID = :2 AND IsActive = 'Y'";
                let rows = db
                    .conn
...
pub fn get_stream_desc(
    ...
    match conn {
        Some(db) => {
            let sql = "select StreamDesc from StreamDef where StreamID = :1";
            let rows = db
                .conn
                .query(sql, &[&id])
...
pub fn get_process_status(
    ...
    match conn {
        Some(db) => {
            let sql = "select ProcessStatus from RunControl where BatchID = :1 AND StreamID = :2 AND FlowID = :3 AND ProcessID = :4 AND AsOnDate = :5 AND PoolID = :6";
            let rows = db
                .conn
                .query(
                    sql,
...
pub fn check_worker_status(
    ...
    match conn {
        Some(db) => {
            let sql = "select ProcessStatus from RunControl where ExecutorID = :1 AND PoolID = :2 AND rownum = 1
                        ORDER BY RunID DESC";
            let rows = db
                .conn
                .query(sql, &[&exe_id, &pool_id])
...
pub fn get_executor_connection_url(
    ...
    match conn {
        Some(db) => {
            let sql = "select ExecutorURL from ExecutorDef where ExecutorID = :1 AND PoolID = :2";
            let rows = db
                .conn
                .query(sql, &[&exe_id, &pool_id])
...
pub fn get_batches(
    ...
    match conn {
        Some(db) => {
            let sql = "select BatchID,BatchName from BatchDef";
            let rows = db
                .conn
                .query(sql, &[])
...
pub fn get_streams(
    ...
    for id in stream_id {
        match conn {
            Some(db) => {
                let sql = "select StreamName from StreamDef where StreamID = :1";
                let rows = db
                    .conn
                    .query(sql, &[&id])
...
pub fn get_batch_status_latest(
    ...
    let batch_id = get_batch_id_latest(path.0, run_id, pool.clone()).unwrap_or(0);
    let ason = fetch_latest_ason(path.0, pool.clone())
        .date()
        .format("%d-%m-%Y")
        .to_string();
...
pub fn get_batch_full_status(
    ...
    let batch_lock_status = get_lock_status(&info.batch_id, &asondate, &pool);
    ...
    for stream_id in streams {
        ...
        for flow in stream.flows {
            ...
            for process in flow.process {
                ...
                match conn {
                    Some(db) => {
                        let sql = "select ProcessStartTime,ProcessEndTime,ProcessStatus,ProcessSummary from RunControl where BatchID = :1 AND StreamID = :2 AND PoolID = :3 AND AsOnDate = :4 AND FlowID = :5 AND ProcessID = :6";
                        let rows = db
                            .conn
                            .query(
                                sql,
        ...
        let tt_by_stream_secs_act = stream_end_time
            ...
            .num_seconds();
        let mut final_stream_status = "NOT STARTED";
        let mut succ_flow_count = 0;
        for status_data in &flow_full_status {
            if status_data.flow_status == "FAIL" {
                final_stream_status = "FAIL";
                break;
            }
            if status_data.flow_status == "PROCESSING" {
                final_stream_status = "PROCESSING";
                break;
            }
            if status_data.flow_status == "NOT STARTED".to_string() {
                continue;
            }
            succ_flow_count += 1;
        }
        if succ_flow_count == flow_full_status.len() {
            final_stream_status = "SUCCESS";
        }
        let stream_full_status = StreamFullStatus {
            ...
            stream_status: final_stream_status.to_string(),
...
pub fn get_batch_id_latest(
    ...
    match conn {
        Some(db) => {
            let sql = "select BatchID from RunControl where RunID = :1 AND PoolID = :2";
            let rows = db
                .conn
                .query(sql, &[&run_id, &pool_id])
...
fn status_by_id(
    ...
    for stream_id in streams {
        ...
        if stream_status != "NOT STARTED" && stream_status != "PROCESSING" {
...
pub fn get_lastest_ason(
    ...
    let asondate = fetch_latest_ason(path.0, pool);
...
fn fetch_latest_ason(
    ...
    match conn {
        Some(db) => {
            let sql = "select max(AsOnDate) from RunControl where PoolID = :1";
            let rows = db
                .conn
                .query(sql, &[&pool_id])
                .expect("Query Failed to Fetch max(AsOnDate) from DB.");
...
pub fn process_executed(
    ...
    match conn {
        Some(db) => {
            let sql = "select ProcessStatus from RunControl where BatchID = :1 AND StreamID = :2 AND FlowID = :3 AND ProcessID = :4 AND AsOnDate = :5 AND RunID = :6 AND PoolID = :7";
            let rows = db
                .conn
                .query(
                    sql,
...
pub fn check_additional_failed_processes(
    ...
    match conn {
        Some(db) => {
            let sql =
                "select ProcessStatus,StreamID,FlowID,ProcessID from RunControl where RunID = :1 AND PoolID = :2";
            let rows = db
                .conn
                .query(sql, &[&trigger_info.run_id, &pool_id])
...
fn get_stream_dep(
    ...
    match conn {
        Some(db) => {
            let sql =
                "select StreamDep from BATCHSTREAM where BatchID = :1 AND StreamID = :2 AND IsActive = 'Y'";
            let rows = db
                .conn
...
pub fn get_lock_status(
    ...
    let conn = &pool
        ...
        .expect("Could not fetch db connection from connection pool!")
        .conn;
    let mut application_id: String = String::new();
    let mut is_locked_status: String = String::new();
    match conn {
        Some(db) => {
            let sql = "select ApplicationId from BatchApplicationDef where BatchId = :1";
            let rows = db
                .conn
                .query(sql, &[batch_id])
                .expect("Query Failed to Fetch BatchApplicationDef Data from DB.");
            for row_result in &rows {
                let row = row_result.expect("Failed to read query output.");
                application_id = row.get("ApplicationId").unwrap_or("0".to_string());
                let sql = "select \"IsLocked\" from \"ReportsLockInfoAsOn\" where \"ApplicationID\" =  :1 and \"AsOnDate\" = :2";
                let lock_status = db
                    .conn
                    .query(
                        sql,
                        &[&application_id, &ason.date().format("%d-%b-%y").to_string()],
                    )
                    .expect("Query Failed to Fetch ReportsLockInfoAsOn Data from DB.");
                for status in &lock_status {
                    let row = status.expect("Failed to read query output.");
                    is_locked_status = row.get("IsLocked").unwrap_or("N".to_string());
