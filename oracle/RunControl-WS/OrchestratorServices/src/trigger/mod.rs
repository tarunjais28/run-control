...
pub fn fire(poolid: i64, trigger: TriggerInfo, pool: web::Data<Pool<OracleConnectionManager>>) {
    ...
    loop {
        let stream_ids: Vec<i64> =
            db::get_triggerable_stream_ids(poolid, trigger.clone(), pool.clone());
        if stream_ids.len() == 0 {
            println!("No Triggerable Stream!!");
            break;
        }
        for id in stream_ids {
            ...
            let flow_ids: Vec<i64> =
                db::get_triggerable_flow_ids(poolid, stream.clone(), trigger.clone(), pool.clone());
                ...
                if flow_ids.contains(&flow_id) {
                    ...
                    for process in flow.process {
                        ...
                        if process_ids.contains(&process_id) {
                            ...
                            if !db::process_executed(poolid, bullet.clone(), pool.clone()) {
        ...
        if db::get_batch_status(poolid, trigger.clone(), pool.clone()) {
            break;
        }
    }
