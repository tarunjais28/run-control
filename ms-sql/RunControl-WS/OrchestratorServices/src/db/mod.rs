...
pub fn get_last_assigned_run_id() -> Result<i64, &'static str> {
    ...
    let sql_cmd = "select max(RunID) as RunID from RunControl;";
    let mut result = 0;
    match stmt
        .exec_direct(&sql_cmd)
        ...
        Data(mut stmt) => {
            while let Some(mut cursor) = stmt.fetch().expect("Cannot read output of query.") {
                    ...
                    .expect("Cannot read row data from query.")
                {
                    ...
                    None => {
                        println!("No runs initiated till now or Run details are not available.");
                        // Default value 0
...
pub fn clear_last_run_det(trigger_info: TriggerInfo, poolid: i64) {
    let con_str = "Driver={ODBC Driver 17 for SQL Server};Server=10.81.4.51;Database=runcontrol;UID=rcuser;PWD=db@123;";
    let env = create_environment_v3()
        .map_err(|e| e.unwrap())
        .expect("Cannot create DB environment.");
...
pub fn get_triggerable_stream_ids(trigger_info: TriggerInfo) -> Vec<i64> {
    let con_str = "Driver={ODBC Driver 17 for SQL Server};Server=10.81.4.51;Database=runcontrol;UID=rcuser;PWD=db@123;";
    ...
    for id in trigger_info.trigger.stream_ids.clone() {
        let env = create_environment_v3()
            ...
            .expect("Cannot create DB environment.");
        let conn = env
            .connect_with_connection_string(&con_str.clone())
            .expect("Cannot establish a DB connection.");
        let stmt = Statement::with_parent(&conn)
            .expect("Cannot create a statement instance to run queries.");
        let sql_cmd = format!(
            "select cast(StreamDep as varchar(max)) from BATCHSTREAM where BatchID = {} AND StreamID = {};",
...
pub fn get_stream_desc(asondate: String, id: i64) -> Option<StreamDef> {
    let con_str = "Driver={ODBC Driver 17 for SQL Server};Server=10.81.4.51;Database=runcontrol;UID=rcuser;PWD=db@123;";
...
pub fn get_triggerable_flow_ids(stream: StreamDef, trigger: TriggerInfo) -> Vec<i64> {
    ...
    for flow in &stream.flows {
        ...
        for dep in &flow.flowDependencies {
            ...
            if !get_flow_status(trigger.clone(), dep_flow_desc, s_id, "SUCCESS".to_string()) {
                status = false;
            }
        }
        if status {
...
pub fn get_triggerable_process_ids(s_id: i64, flow: FlowDef, trigger: TriggerInfo) -> Vec<i64> {
    ...
    for process in &flow.process {
        ...
        for dep in &process.processDependencies {
            ...
            if !get_process_status(trigger.clone(), p_id, f_id, s_id, "SUCCESS".to_string()) {
                status = false;
            }
        }
        if status {
...
pub fn get_stream_status(trigger_info: TriggerInfo, s_id: i64, key_status: String) -> bool {
    let mut status = true;
    let stream: StreamDef = get_stream_desc(trigger_info.trigger.as_on_date.clone(), s_id)
    ...
    for flow in stream.flows {
        if !get_flow_status(trigger_info.clone(), flow, s_id, key_status.clone()) {
            status = false;
        }
    }
    status
...
pub fn get_process_status(
    ...
    let as_on_date =
        chrono::NaiveDate::parse_from_str(&trigger_info.trigger.as_on_date, "%d-%m-%Y")
            ...
            .and_hms(0, 0, 0);
    let sql_cmd = format!("select ProcessStatus from RunControl where BatchID = {} AND StreamID = {} AND FlowID = {} AND ProcessID = {} AND AsOnDate = {};", trigger_info.trigger.batch_id.to_string(), s_id.to_string(), f_id.to_string(), p_id.to_string(), as_on_date.timestamp());
    match stmt
        .exec_direct(&sql_cmd)
        .expect("Failed to execute a sql cmd!!")
    {
...
pub fn get_batch_status(trigger: TriggerInfo) -> bool {
    let mut status = true;
    for id in &trigger.trigger.stream_ids {
        let stream: StreamDef = get_stream_desc(trigger.trigger.as_on_date.clone(), *id)
            .expect("Cannot fetch stream description from db.");
        for flow in stream.flows {
            for process in flow.process {
                let bullet = Bullet {
...
pub fn get_pool_id(id: i64) -> i64 {
    ...
    let stmt =
        Statement::with_parent(&conn).expect("Cannot create a statement instance to run queries.");
    let sql_cmd = format!("select PoolID from BatchDef where BatchID = {}", id);
        ...
        .expect("Failed to execute a sql cmd!!")
    {
        Data(mut stmt) => {
            while let Some(mut cursor) = stmt.fetch().expect("Cannot read output of query.") {
                ...
                {
                    Some(val) => {
                        pool_id = val.parse::<i64>().unwrap_or(0);
                    }
                    None => {
                        println!("Query \"{}\" executed, no data returned", sql_cmd);
                    }
...
pub fn get_actual_worker_id(pool_id: i64, logical_worker_id: i64) -> i64 {
    let con_str = "Driver={ODBC Driver 17 for SQL Server};Server=10.81.4.51;Database=runcontrol;UID=rcuser;PWD=db@123;";
    let env = create_environment_v3()
        .map_err(|e| e.unwrap())
        .expect("Cannot create DB environment.");
    ...
    let stmt =
        Statement::with_parent(&conn).expect("Cannot create a statement instance to run queries.");
    let sql_cmd = format!(
        "select ExecutorID from ExecutorPool where PoolID = {}",
        pool_id
    );
        ...
        .expect("Failed to execute a sql cmd!!")
    {
        Data(mut stmt) => {
            while let Some(mut cursor) = stmt.fetch().expect("Cannot read output of query.") {
                ...
                {
                    Some(val) => {
                        let exe_id: i64 = val.parse::<i64>().unwrap_or(0);
...
pub fn check_worker_status(id: i64) -> bool {
    let con_str = "Driver={ODBC Driver 17 for SQL Server};Server=10.81.4.51;Database=runcontrol;UID=rcuser;PWD=db@123;";
    let env = create_environment_v3()
        .map_err(|e| e.unwrap())
        .expect("Cannot create DB environment.");
    ...
    let stmt =
        Statement::with_parent(&conn).expect("Cannot create a statement instance to run queries.");
    let sql_cmd = format!(
        "select ProcessStatus from RunControl where ExecutorID = {} ORDER BY RunID DESC;",
        id
    );
    let mut status = true;
    match stmt
...
pub fn get_executor_connection_url(id: i64) -> String {
    let con_str = "Driver={ODBC Driver 17 for SQL Server};Server=10.81.4.51;Database=runcontrol;UID=rcuser;PWD=db@123;";
    let env = create_environment_v3()
        .map_err(|e| e.unwrap())
        .expect("Cannot create DB environment.");
    ...
    let stmt =
        Statement::with_parent(&conn).expect("Cannot create a statement instance to run queries.");
    let sql_cmd = format!(
        "select ExecutorURL from ExecutorDef where ExecutorID = {};",
        id
    );
...
pub fn add_to_run_control(pool_id: i64, status: &str, worker_id: i64, info: Bullet) {
    let con_str = "Driver={ODBC Driver 17 for SQL Server};Server=10.81.4.51;Database=runcontrol;UID=rcuser;PWD=db@123;";
...
pub fn get_batches(path: Path<(i64,)>) -> Result<HttpResponse, Error> {
    let con_str = "Driver={ODBC Driver 17 for SQL Server};Server=10.81.4.51;Database=runcontrol;UID=rcuser;PWD=db@123;";
    let env = create_environment_v3()
        .map_err(|e| e.unwrap())
        .expect("Cannot create DB environment.");
    ...
    let stmt =
        Statement::with_parent(&conn).expect("Cannot create a statement instance to run queries.");
    let sql_cmd = format!(
        "select BatchID,BatchName from BatchDef where PoolID = {};",
        path.0
    );
...
pub fn get_streams(_path: Path<(i64,)>, info: Json<StatusReq>) -> Result<HttpResponse, Error> {
    let con_str = "Driver={ODBC Driver 17 for SQL Server};Server=10.81.4.51;Database=runcontrol;UID=rcuser;PWD=db@123;";
    let env = create_environment_v3()
        .map_err(|e| e.unwrap())
        .expect("Cannot create DB environment.");
    ...
    let sql_cmd = format!("select StreamID from BatchStream where BatchID = {};", id);
    let stmt =
        Statement::with_parent(&conn).expect("Cannot create a statement instance to run queries.");
    match stmt
        .exec_direct(&sql_cmd)
...
pub fn get_batch_status_latest(path: Path<(i64,)>) -> Result<HttpResponse, Error> {
    let run_id = get_last_assigned_run_id().unwrap_or(0);
    let batch_id = get_batch_id_latest(run_id).unwrap_or(0);
    let ason = fetch_latest_ason().date().format("%d-%m-%Y").to_string();
    let resp = status_by_id(path.0, batch_id, ason);
    let batches_info = json::encode(&resp).expect("Cannot serialize as JSON.");
    Ok(HttpResponse::Ok()
        .content_type("application/json")
        .body(batches_info))
...
pub fn get_batch_full_status(
    ...
    for stream_id in streams {
        let stream: StreamDef = get_stream_desc(asondate.format("%d-%m-%Y").to_string(), stream_id)
            .expect("Cannot fetch stream description from db.");
        ...
        for flow in stream.flows {
            ...
            for process in flow.process {
                ...
                let stmt_2 = Statement::with_parent(&conn)
                    .expect("Cannot create a statement instance to run queries.");
                let sql_cmd_2 = format!("select ProcessStartTime,ProcessEndTime,cast(ProcessSummary as varchar(max)),ProcessStatus from RunControl where BatchID = {} AND StreamID = {} AND PoolID = {} AND AsOnDate = {} AND FlowID = {} AND ProcessID = {};", info.batch_id,stream_id,path.0,asondate.timestamp(),flow.flowId,process.processId);
                match stmt_2
                    .exec_direct(&sql_cmd_2)
                    .expect("Failed to execute a sql cmd!!")
                {
        ...
        let stream_full_status = StreamFullStatus {
            stream_id: stream_id,
            stream_name: get_stream_name(stream_id),
...
pub fn get_batch_id_latest(run_id: i64) -> Result<i64, &'static str> {
    let con_str = "Driver={ODBC Driver 17 for SQL Server};Server=10.81.4.51;Database=runcontrol;UID=rcuser;PWD=db@123;";
    let env = create_environment_v3()
        .map_err(|e| e.unwrap())
        .expect("Cannot create DB environment.");
    ...
    let stmt =
        Statement::with_parent(&conn).expect("Cannot create a statement instance to run queries.");
    let sql_cmd = format!("select BatchID from RunControl where RunID = {};", run_id);
    match stmt
        .exec_direct(&sql_cmd)
        .expect("Failed to execute a sql cmd!!")
    {
...
fn status_by_id(pool_id: i64, batchid: i64, ason: String) -> RunStatus {
    let con_str = "Driver={ODBC Driver 17 for SQL Server};Server=10.81.4.51;Database=runcontrol;UID=rcuser;PWD=db@123;";
    let env = create_environment_v3()
        .map_err(|e| e.unwrap())
        .expect("Cannot create DB environment.");
    ...
    for stream_id in streams {
        ...
        let stream: StreamDef = get_stream_desc(ason.clone(), stream_id)
            .expect("Cannot fetch stream description from db.");
...
pub fn get_lastest_ason(_path: Path<(i64,)>) -> Result<HttpResponse, Error> {
    let asondate = fetch_latest_ason();
...
pub fn get_batch_lastest_ason(
    ...
    let asondate = fetch_latest_ason_by_batch(path.0, body.batch_id);
    let latest_ason = asondate.date().format("%Y-%m-%d").to_string();
    let ason = AsOn {
        as_on_date: latest_ason,
    };
...
pub fn get_batch_last_success_ason(
    ...
    let asondate = fetch_batch_last_success_ason(path.0, body.batch_id);
    let latest_ason = asondate.date().format("%Y-%m-%d").to_string();
    Ok(HttpResponse::Ok()
...
fn fetch_latest_ason() -> DateTime<Utc> {
    let con_str = "Driver={ODBC Driver 17 for SQL Server};Server=10.81.4.51;Database=runcontrol;UID=rcuser;PWD=db@123;";
    ...
    let stmt =
        Statement::with_parent(&conn).expect("Cannot create a statement instance to run queries.");
    let sql_cmd = format!("select max(AsOnDate) from RunControl;");
    match stmt
        .exec_direct(&sql_cmd)
        .expect("Failed to execute a sql cmd!!")
    {
...
fn fetch_latest_ason_by_batch(pool_id: i64, b_id: i64) -> DateTime<Utc> {
    let con_str = "Driver={ODBC Driver 17 for SQL Server};Server=10.81.4.51;Database=runcontrol;UID=rcuser;PWD=db@123;";
    let mut asondate: DateTime<Utc> = Utc::now();
    let env = create_environment_v3()
        .map_err(|e| e.unwrap())
...
fn fetch_batch_last_success_ason(pool_id: i64, b_id: i64) -> DateTime<Utc> {
    let con_str = "Driver={ODBC Driver 17 for SQL Server};Server=10.81.4.51;Database=runcontrol;UID=rcuser;PWD=db@123;";
...
pub fn process_executed(bullet: Bullet) -> bool {
    let con_str = "Driver={ODBC Driver 17 for SQL Server};Server=10.81.4.51;Database=runcontrol;UID=rcuser;PWD=db@123;";
    let mut status = false;
    ...
    let stmt =
        Statement::with_parent(&conn).expect("Cannot create a statement instance to run queries.");
    let sql_cmd = format!(
        "select ProcessStatus from RunControl where BatchID = {} AND StreamID = {} AND FlowID = {} AND ProcessID = {} AND AsOnDate = {} AND RunID = {};",
        bullet.batch_id,
        bullet.stream_id,
        bullet.flow_id,
        bullet.process_id,
...
pub fn check_additional_failed_processes(pool_id: i64, trigger_info: TriggerInfo) {
    let con_str = "Driver={ODBC Driver 17 for SQL Server};Server=10.81.4.51;Database=runcontrol;UID=rcuser;PWD=db@123;";
    ...
    let stmt =
        Statement::with_parent(&conn).expect("Cannot create a statement instance to run queries.");
    let sql_cmd = format!(
        "select ProcessStatus,StreamID,FlowID,ProcessID from RunControl where RunID = {};",
        trigger_info.run_id
    );
    ...
    loop {
        for id in trigger_info.trigger.stream_ids.clone() {
            let dep_stream_ids: Vec<i64> = get_stream_dep(trigger_info.trigger.batch_id, id);
            for dep_id in dep_stream_ids {
                if failed_stream_ids.contains(&dep_id) {
                    let stream: StreamDef = get_stream_desc(ason.clone(), id)
                        .expect("Cannot fetch stream description from db.");
                    for flow in stream.flows {
                        for process in flow.process {
        ...
        for id in &failed_stream_ids {
            let stream: StreamDef = get_stream_desc(ason.clone(), *id)
                .expect("Cannot fetch stream description from db.");
            println!("Failed Flow ID's: {:#?}", failed_flow_ids);
            for flow in stream.flows {
    ...
    for ele in fail_set {
        ...
        if !is_fail && !is_success {
            add_to_run_control(pool_id, status, 0, info);
        }
    }
}

fn get_stream_dep(batchid: i64, stream_id: i64) -> Vec<i64> {
    let con_str = "Driver={ODBC Driver 17 for SQL Server};Server=10.81.4.51;Database=runcontrol;UID=rcuser;PWD=db@123;";
    let env = create_environment_v3()
        .map_err(|e| e.unwrap())
        .expect("Cannot create DB environment.");
...
fn get_stream_name(stream_id: i64) -> String {
    let con_str = "Driver={ODBC Driver 17 for SQL Server};Server=10.81.4.51;Database=runcontrol;UID=rcuser;PWD=db@123;";
    let env = create_environment_v3()
        .map_err(|e| e.unwrap())
        .expect("Cannot create DB environment.");
...
fn get_stream_status_by_batch(pool_id: i64, batch_id: i64, ason: String, stream_id: i64) -> String {
    let batch_status = status_by_id(pool_id, batch_id, ason);
